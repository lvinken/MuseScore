...
Score::FileError MusicXMLParserPass2::parse(QIODevice* device, const musicxml::score_partwise& score_partwise)
{
    qDebug("MusicXMLParserPass2::parse()");
    _e.setDevice(device);
    Score::FileError res = parse(score_partwise);
...
/**
 Start the parsing process, TODO (?): after verifying the top-level node is score-partwise
 */

Score::FileError MusicXMLParserPass2::parse(const musicxml::score_partwise& score_partwise)
{
    scorePartwise(score_partwise);
...
void MusicXMLParserPass2::measure(const musicxml::measure1& measure,
      ...
      // TODO: port remaining original code
                  ...
                  harmony(partId, currentMeasure, time + mTime);
                  ...
                  Note* n = note(partId, currentMeasure, time + mTime, time + prevTime, missingPrev, dura, missingCurr, cv, gcl, gac, beam, fbl, alt, tupletStates, tuplets);
...
void MusicXMLParserPass2::attributes(const musicxml::attributes& attributes, const QString& partId, Measure* measure, const Fraction& tick)
{
    ...
    if (attributes.time().size() > 0) {
        time(attributes.time()[0], partId, measure, tick);
    }
...
QString clef_sign_to_string(::musicxml::clef_sign v)
{
    if (v == ::musicxml::clef_sign::G) {
        return "G";
    }
    else if (v == ::musicxml::clef_sign::F) {
        return "F";
    }
    else {
        return "clef_sign_unknown";
    }
}
...
void MusicXMLParserPass2::clef(const musicxml::clef& mxmlclef, const QString& partId, Measure* measure, const Fraction& tick)
      {
      Part* part = _pass1.getPart(partId);
      Q_ASSERT(part);
      ...
      if (clefno <= 0 || clefno > part->nstaves()) {
            ...
            _logger->logError(QString("invalid clef number '%1'").arg(clefno), &_e);
            clefno = 1;
            }
      // convert to 0-based
      clefno--;
      ...
      QString c { clef_sign_to_string(mxmlclef.sign()) };
      int i = 0; // TODO
      int line = -1;
      if (mxmlclef.line()) {
          line = *mxmlclef.line();
...
void MusicXMLParserPass2::time(const musicxml::time& mxmltime, const QString& partId, Measure* measure, const Fraction& tick)
      {
      QString beats;
      if (mxmltime.beats().size() > 0) {
          beats = mxmltime.beats()[0].data();
      }
      QString beatType;
      if (mxmltime.beat_type().size() > 0) {
          beatType = mxmltime.beat_type()[0].data();
      }
...
Note* MusicXMLParserPass2::note(const musicxml::note& mxmlnote,
      ...
      if (!mxmlnote.grace()) {
          for (const auto& mxmlLyric : mxmlnote.lyric()) {
...
void MusicXMLParserLyric::parse(const musicxml::lyric& mxmlLyric)
      ...
      const QString lyricNumber { "1" }; // TODO = _e.attributes().value("number").toString();
      ...
      for (const auto& mxmlText : mxmlLyric.text()) {
          //qDebug("lyric '%s'", mxmlText.data());
          formattedText += mxmlText.data();
      }
