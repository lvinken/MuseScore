...
public:
      ...
      int numberChordRests(int s) const { return (s >= 0 && s < MAX_MXML_STAVES) ? _chordRests[s] : 0; }
      ...
      void setVoice(int s, int v) { if (s >= 0 && s < MAX_MXML_STAVES) _voices[s] = v; }
      int voice(int s) const { return (s >= 0 && s < MAX_MXML_STAVES) ? _voices[s] : -1; }
      void setOverlap(bool b) { _overlaps = b; }
      bool overlaps() const { return _overlaps; }
      void setStaffAlloc(int s, int i) { if (s >= 0 && s < MAX_MXML_STAVES) _staffAlloc[s] = i; }
      int staffAlloc(int s) const { return (s >= 0 && s < MAX_MXML_STAVES) ? _staffAlloc[s] : -1; }
      QString toString() const;
private:
      int _chordRests[MAX_MXML_STAVES];      ///< The number of chordrests on each MusicXML staff
      ...
      int _staffAlloc[MAX_MXML_STAVES];      ///< For overlapping voices: voice is allocated on these staves (note: -2=unalloc -1=undef 1=alloc)
      int _voices[MAX_MXML_STAVES];          ///< For every voice allocated on the staff, the voice number
      };
